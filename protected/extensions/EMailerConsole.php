...
class EMailerConsole {
    ...
    public function getView($view, $vars = array(), $layout = null) {
        ...
        $viewPath = Yii::getPathOfAlias(Yii::app()->mailer->pathViews . '.' . 'alertsCron') . '.php';

        $body = CBaseController::renderInternal($viewPath, array_merge($vars, array('content' => $this->_myMailer)), true);
